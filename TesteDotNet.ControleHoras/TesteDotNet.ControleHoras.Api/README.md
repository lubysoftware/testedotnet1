...
#### Identificação:
Nome: Vinícius Rafael Migliorança<br/>
E-mail: vrmvinicius@gmail.com

#### Observação:
Implementei uma parte do frontend porém não dispus de tempo suficiente para completá-lo. Optei por fazer em Blazor
na qual já havia feito algumas implementações. Na parte aqui da API, teriam muitas questões de validação e modelagem
a serem melhoradas, porém a arquitetura inicial bem como as funcionalidades básicas para o início da implementação
de um projeto estão prontas. Utilizei um banco sql server local. Qualquer dúvida estou a disposição.