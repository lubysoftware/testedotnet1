...
namespace API.Controllers
{
    ...
    public class AuthController : MainController
    {
        ...
        public AuthController(INotifier notificador, SignInManager<IdentityUser> signInManager, UserManager<IdentityUser> userInManager, IOptions<AppSettings> appSettings, IUser user)
        ...
        private async Task<LoginResponseViewModel> GerarJwt(string email)
        {
            ...
            claims.Add(new Claim(JwtRegisteredClaimNames.Nbf, ToUnixExpochDate(DateTime.UtcNow).ToString(), ClaimValueTypes.Integer64));
            ...
            var response = new LoginResponseViewModel
            {
                ...
                ExpiresIn = TimeSpan.FromMinutes(_appSettings.ExpiracaoMinutos).TotalMinutes,
        ...
        private static long ToUnixExpochDate(DateTime date) => (long)Math.Round((date.ToUniversalTime() - new DateTimeOffset(1970, 1, 1, 0, 0, 0, TimeSpan.Zero)).TotalSeconds);
    }
}
