...
namespace Dommel
{
    public static partial class DommelMapper
    {
        ...
        /// <summary>
        /// Generate the Where clausule matching the specified predicate.
        /// </summary>
        /// <typeparam name="TEntity">The type of the entity.</typeparam>
        /// <param name="connection">The connection to the database. This can either be open or closed.</param>
        /// <param name="predicate">A predicate to filter the results.</param>
        /// <param name="parameters">Parameters to where</param>
        /// <returns>Where clausule</returns>
        public static string BuildWhereClausule<TEntity>(this IDbConnection connection, Expression<Func<TEntity, bool>> predicate, out DynamicParameters parameters)
        {
            var sql = BuildSelectSql(connection, predicate, out parameters);
            return " " + sql.Substring(sql.ToUpper().IndexOf("WHERE"));
        }
