    ...
    public class SqlExpression<TEntity>
    {
        ...
        protected virtual object VisitCallExpression(MethodCallExpression expression)
        {
            ...
            switch (method)
            {
                ...
                /* begin - RABBOT */
                case "replace":
                    return VisitReplaceExpression(expression);
                /* end - RABBOT */
                default:
                    break;
            }
        ...
        /// <summary>
        /// Processes a replace expression for string.
        /// </summary>
        /// <param name="expression">The method call expression.</param>
        /// <returns>The result of the processing.</returns>
        protected virtual object VisitReplaceExpression(MethodCallExpression expression)
        {
            Expression expObject = expression.Object;

            var column = VisitExpression(expObject);
            if (expression.Arguments.Count != 2)
            {
                throw new ArgumentException("Replace-expression should contain exactly two arguments.", nameof(expression));
            }

            var oldValue = VisitExpression(expression.Arguments[0]);
            var newValue = VisitExpression(expression.Arguments[1]);

            if (expObject.NodeType == ExpressionType.MemberAccess)
            {
                if (((MemberExpression)expObject).Expression?.NodeType == ExpressionType.Parameter)
        ...
        protected virtual object VisitBinary(BinaryExpression expression)
        {
            ...
            else
            {
                ...
                {
                    // Execute .Net method before to send
                    MethodCallExpression methodExpression = (MethodCallExpression)right;
                    right = Expression.Lambda(methodExpression).Compile().DynamicInvoke();
                }
                else if (expression.Right.NodeType == ExpressionType.Call)
                {
                    if (((MethodCallExpression)expression.Right).Method.Name.ToLower() == "replace")
            ...
            // Keep expression hierarchy
            return $"({left} {operand} {right})";
            //return $"{left} {operand} {right}"; // padrão do Dommel
        ...
        protected virtual string MemberToColumn(MemberExpression expression) =>
            ...
            $"{Resolvers.Table(typeof(TEntity), SqlBuilder)}.{Resolvers.Column((PropertyInfo)expression.Member, SqlBuilder)}";
            //Resolvers.Column((PropertyInfo)expression.Member, SqlBuilder); // padrão do Dommel
